...
def list_books(request):
 mybook = Book(title = 'Continuous Delivery', author = 'J.Humble and D. Farley', edition
  = 1)
...
def complex_query(request):
  ...
  False).filter(title__icontains='and').filter(edition__gte = 2012).exclude(price__lte = 100)[:10]
  if len(mybooks)>=1:
    return render(request, 'bookmodule/bookList.html', {'books':mybooks})
  else:
    return render(request, 'bookmodule/index.html')
...
def text2(request):
 return render(request, 'bookmodule/html5/text2.html')
def table(request):
...
def search(request):
 ...
 return render(request, 'bookmodule/html5/search.html')
